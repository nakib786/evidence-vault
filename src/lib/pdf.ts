...
/** One item's own sections: what it is, how it's classified, its account/transcript, and its content. */
async function renderItemSections(doc: Doc, record: EvidenceRecord, index: number, total: number): Promise<void> {
  const { details } = record;
  const multi = total > 1;
  const h = (n: number, text: string): string => (multi ? text : `${n}. ${text}`);
  ...
  if (record.captureMeta) {
    for (const fact of describeCaptureMeta(record.captureMeta)) {
      doc.field(fact.label, fact.value);
    }
  }

  // ---- Classification -----------------------------------------------------
  doc.heading(h(2, 'Reporter’s classification'), size);
  doc.body(
  ...
  if (details.note.trim()) {
    doc.heading(h(3, 'Reporter’s account'), size);
  ...
  // ---- The evidence itself ------------------------------------------------
  doc.heading(h(5, 'The recorded content'), size);
  if (record.kind === 'video') {
    doc.body(
  ...
  doc.pdf.addPage();
  doc.gap(0);
  doc.heading(h(6, 'Integrity and timestamp proof'), size);
  ...
  if (record.proof) {
    doc.field('Proof submitted', humanTime(record.proof.submittedAt, record.timeZone));
    doc.field(
      'Calendar servers',
      record.proof.calendars.filter((c) => c.ok).map((c) => c.calendar).join('\n') || 'None responded',
    );
    doc.field('Attestations pending at', record.proof.pendingUris.join('\n') || '—');
    doc.body(
...
/**
 * Build the evidence report for one or more captured items.
 *
 * A single item gets the original numbered layout unchanged (§1–§8) — this is still the
 * path the vault re-downloads through, and it should never quietly change shape underneath
 * an already-saved record. Several items share one report instead: each gets its own
 * numbered section with its own classification, account, transcript and content — a video
 * included exactly the way a single-item report already handles one, as a timecoded
 * contact sheet, since a PDF cannot embed a playable recording — and the verification and
 * limits sections are stated once at the end rather than repeated per item.
 */
export async function buildPackageReportPdf(records: EvidenceRecord[]): Promise<Blob> {
  if (records.length === 0) throw new Error('buildPackageReportPdf requires at least one record');
  ...
  doc.pdf.text(multi ? 'Evidence Report' : 'Evidence Record', MARGIN, doc.cursor);
  doc.gap(26);
  doc.body(
    multi
      ? `A tamper-evident record of ${records.length} documented items, produced on the reporter’s ` +
          'own device. Each item’s cryptographic digest was calculated at the moment of capture and ' +
          'submitted to independent OpenTimestamps calendar servers, which register it on a public ' +
          'blockchain ledger.'
  ...
  for (let i = 0; i < records.length; i += 1) {
    if (i > 0) doc.pdf.addPage();
    await renderItemSections(doc, records[i], i + 1, records.length);
  }

  // ---- Verification -------------------------------------------------------
  doc.heading(multi ? 'How to verify this report' : '7. How to verify this record', multi ? 13 : 15);
  doc.body(
    'Verification does not depend on this application, its authors, or any server they control. ' +
      'Anyone holding an original file and its .ots proof can check it with the standard ' +
      'OpenTimestamps tooling:',
    { muted: true, size: 9 },
  );
  doc.gap(8);
  doc.body('1.  Recompute the digest of the original file and confirm it matches the value stated for it above.');
  ...
  // ---- Limits -------------------------------------------------------------
  doc.heading(
    multi ? 'What this report does and does not establish' : '8. What this record does and does not establish',
    13,
  );
  doc.body('It establishes:', { muted: true, size: 9 });
  doc.body(
    '•  That each exact file existed no later than the timestamp confirmed on the public ' +
      'blockchain.\n' +
      '•  That no file has been altered since — any change, however small, produces a ' +
      'different digest and breaks its proof.',
  );
  doc.gap(6);
  ...
  doc.gap(6);
  if (records.some((r) => r.source === 'upload')) {
    doc.body(
      records.length === 1
        ? 'Where the capture method above is recorded as "Existing file", note that the file was ' +
            'already present on the device before this application saw it, and could in principle ' +
            'have been edited beforehand. Records made by live capture do not have that gap.'
        : 'Where an item’s capture method above is recorded as "Existing file", note that the file was ' +
            'already present on the device before this application saw it, and could in principle ' +
            'have been edited beforehand. Items made by live capture do not have that gap.',
      { muted: true, size: 9 },
    );
  }

  doc.footers(multi ? `Evidence report — ${records.length} items` : `Evidence record ${records[0].id}`);
  return doc.pdf.output('blob');
}

export async function buildReportPdf(record: EvidenceRecord): Promise<Blob> {
  return buildPackageReportPdf([record]);
}
